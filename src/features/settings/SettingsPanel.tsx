...
export function SettingsPanel({
  ...
  async function setTheme(theme: ThemeMode) {
    ...
    try {
      ...
      setStatus(getUserMessage(error, 'Der Darstellungsmodus konnte nicht gespeichert werden.'))
    }
  }
  ...
  async function sync() {
    ...
    try {
      ...
      setStatus(getUserMessage(error, 'Synchronisierung nicht möglich.'))
    } finally {
      setSyncing(false)
    }
  }
      ...
      <Modal open={open && !editingProfile} title="Einstellungen" onClose={onClose}>
        {!demo && (
          <Card className="stack">
            ...
            <Button variant="secondary" full onClick={() => setEditingProfile(true)}>
        ...
        {!demo && (
          <Card className="stack">
            <div className="row">
              ...
              <div>
                <strong>Passwort festlegen</strong>
                <span className="small muted" style={{ display: 'block' }}>
                  Optional für eine zweite Anmeldemöglichkeit
                </span>
              </div>
            </div>
            <Field
            ...
            <Button
              variant="secondary"
              full
              disabled={savingPassword || password.length < 8 || passwordConfirmation.length < 8}
        ...
        <Card className="stack">
          ...
          <div className="segmented">
            <button aria-pressed={settings?.theme === 'light'} onClick={() => void setTheme('light')}>
              <Sun size={16} /> Hell
            </button>
            <button aria-pressed={settings?.theme === 'system'} onClick={() => void setTheme('system')}>
              Auto
            </button>
            <button aria-pressed={settings?.theme === 'dark'} onClick={() => void setTheme('dark')}>
              <Moon size={16} /> Dunkel
            </button>
          </div>
        </Card>
          ...
          <Card className="stack">
            <div className="card__row">
              <div className="row">
                <Cloud size={20} />
                <div>
                  <strong>Datensynchronisierung</strong>
                  <span className="small muted" style={{ display: 'block' }}>
                    {online ? (pending ? `${pending} Änderungen warten` : 'Aktuell') : 'Offline · lokal gesichert'}
                  </span>
                </div>
              </div>
              <span className={`connection-dot ${online ? '' : 'connection-dot--offline'}`} />
            </div>
            <Button variant="secondary" full disabled={!online || syncing} onClick={() => void sync()}>
        ...
        <Button variant={demo ? 'secondary' : 'danger'} full disabled={loggingOut} onClick={() => void logout()}>
          <LogOut size={18} /> {demo ? 'Zur Anmeldung' : loggingOut ? 'Wird abgemeldet …' : 'Abmelden'}
        </Button>
        <p className="auth-note">
          {demo
            ? 'Änderungen in der Demo bleiben ausschließlich auf diesem Gerät.'
            : 'Bont speichert laufende Trainings und Änderungen zuerst lokal. Cloud-Daten werden pro Nutzer durch Zugriffsregeln getrennt.'}
        </p>
      </Modal>
      <ProfileEditor
        key={editingProfile ? 'profile-open' : 'profile-closed'}
        open={open && editingProfile}
        profile={profile}
        settings={settings}
...
function ProfileEditor({
  ...
  return (
    <Modal open={open} title="Profildaten" onClose={onClose}>
      ...
      />
      <SelectField
        label="Biologisches Geschlecht für Berechnung"
        value={sex}
      ...
      <div className="input-row">
        <Field label="Größe (cm)" type="number" value={height} onChange={(event) => setHeight(event.target.value)} />
        <Field
      ...
      >
        ...
        <option value="athlete">Extrem aktiv</option>
      </SelectField>
      <SelectField
        label="Körperfett-Kategorie"
        value={bodyFat}
        onChange={(event) => setBodyFat(event.target.value as BodyFatCategory)}
      >
        ...
        <option value="high">Erhöht</option>
      </SelectField>
      <InfoNote>
        Änderungen aktualisieren nur die vorläufige Kalorienschätzung. Sobald genügend Verlaufsdaten vorhanden sind, hat
        die datenbasierte Schätzung Vorrang.
      </InfoNote>
      {error && (
      ...
      <Button full disabled={saving || !name.trim() || !birthDate || !height || !weight} onClick={() => void save()}>

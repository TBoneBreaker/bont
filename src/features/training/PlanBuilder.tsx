...
export function PlanBuilder(props: PlanBuilderProps) {
  ...
  if (props.existingPlan && !planData)
    return (
      <div className="center-screen">
        <p className="muted">Plan wird geladen …</p>
      </div>
    )
...
function PlanBuilderEditor({
  ...
  const totalSets = useMemo(
    () =>
      days.reduce((sum, day) => sum + day.exercises.reduce((daySum, exercise) => daySum + exercise.targetSets, 0), 0),
    [days],
  )
  const valid =
    name.trim().length >= 2 &&
    days.every(
      (day) => day.name.trim() && day.exercises.length > 0 && day.exercises.every((exercise) => exercise.name.trim()),
    )
  ...
  return (
    <div className="subview">
      <ScreenHeader
        title={templateMode ? 'Vorlage erstellen' : existingPlan ? 'Plan bearbeiten' : 'Plan erstellen'}
        onBack={onCancel}
      />
      <main className="content content--narrow">
        <Card className="stack">
          ...
          <div className="row row--between">
            <span className="muted small">Sätze im gesamten Plan</span>
            <strong>{totalSets} Sätze</strong>
          </div>
        </Card>
          ...
          <Card className="stack" key={day.id}>
            <div className="card__row">
              <div className="row">
                ...
                <h2 style={{ margin: 0 }}>Trainingstag</h2>
              </div>
              <span className="pill">
                {day.exercises.reduce((sum, exercise) => sum + exercise.targetSets, 0)} Sätze
              </span>
            </div>
            <Field
              label="Name"
              placeholder={defaultDayName(dayIndex, split)}
              value={day.name}
                ...
                <div className="exercise-draft" key={exercise.id}>
                  ...
                  <div className="exercise-draft__bottom">
                    <div className="set-counter">
                      ...
                      <button
                        type="button"
                        aria-label={`Sätze für ${exercise.name || `Übung ${exerciseIndex + 1}`} verringern`}
                        disabled={exercise.targetSets <= 1}
                        onClick={() => changeTargetSets(dayIndex, exerciseIndex, -1)}
                      >
                        <Minus size={16} />
                      </button>
                      ...
                      <button
                        type="button"
                        aria-label={`Sätze für ${exercise.name || `Übung ${exerciseIndex + 1}`} erhöhen`}
            ...
            <Button variant="secondary" full onClick={() => addExercise(dayIndex)}>
              <Plus size={18} /> Übung hinzufügen
            </Button>
            <div className="day-volume-summary">
              <div>
                <span>Gesamtvolumen</span>
                <small>{day.name.trim() || `Training ${dayIndex + 1}`}</small>
              </div>
        ...
        <Card>
          <TextareaField
            label="Notizen zum Plan"
            value={notes}
            onChange={setNotes}
            placeholder="Optional: Fokus, Pausenzeiten oder Hinweise …"
          />
        </Card>
        {error && (
        ...
        <Button full disabled={saving} onClick={() => void save()}>
          <Save size={18} />{' '}
          {saving ? 'Wird gespeichert …' : templateMode ? 'Vorlage speichern' : 'Trainingsplan speichern'}
        </Button>
      </main>
    </div>
  )

...
export function useWorkoutData(userId: string, sessionId: string) {
  ...
  const exercises = useLiveQuery(async () => session
    ? (await db.exercises.where('training_day_id').equals(session.training_day_id).toArray()).filter((item) => item.user_id === userId && !item.deleted_at).sort((a, b) => a.order_index - b.order_index)
    : [], [session?.training_day_id, userId], [])
  const sets = useLiveQuery(async () => (await db.workout_sets.where('session_id').equals(sessionId).toArray()).filter((item) => item.user_id === userId && !item.deleted_at).sort((a, b) => a.set_number - b.set_number), [sessionId, userId], [])
...
export function useExerciseProgressData(userId: string, exerciseId: string | undefined) {
  ...
  const allSets = useLiveQuery(async () => exerciseId ? (await db.workout_sets.where('exercise_id').equals(exerciseId).toArray()).filter((item) => item.user_id === userId && !item.deleted_at && item.is_completed) : [], [exerciseId, userId], [])

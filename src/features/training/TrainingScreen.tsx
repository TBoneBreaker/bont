...
export function TrainingScreen({ userId, displayName }: { userId: string; displayName: string }) {
  ...
  async function startWorkout(day: TrainingDay, date: string) {
    ...
    try {
      const dayExercises = exercises
        ...
        .sort((a, b) => a.order_index - b.order_index)
      const session = await startWorkoutCommand({
        userId,
        plan: activePlan,
        day,
  ...
  if (builder) {
    return (
      <PlanBuilder
        ...
        onSaved={() => {
          setBuilder(null)
          setView(builder.template ? 'templates' : 'overview')
        }}
      />
    )
  }
    ...
    return (
      <div className="subview">
        <ScreenHeader
          ...
          action={
            <IconButton label="Vorlage hinzufügen" onClick={() => setBuilder({ template: true })}>
              <Plus size={20} />
            </IconButton>
          }
        />
          ...
          {templates.length === 0 ? (
            <Card>
              <EmptyState
                ...
                action={
                  <Button onClick={() => setBuilder({ template: true })}>
                    <Plus size={18} /> Vorlage hinzufügen
                  </Button>
                }
              />
            </Card>
            ...
            templates.map((template) => (
              <Card key={template.id} className="stack">
                <div className="card__row card__row--top">
                  <div>
                    <span className="eyebrow">{template.split_size}er-Split</span>
                    <h2>{template.name}</h2>
                    <p className="muted small">{template.notes || 'Keine Notizen'}</p>
                  </div>
                  <IconButton label="Vorlage bearbeiten" onClick={() => setBuilder({ plan: template, template: true })}>
                    <MoreHorizontal size={20} />
                  </IconButton>
                </div>
                <Button full onClick={() => void applyTemplate(template)}>
                  Als Plan verwenden <ChevronRight size={18} />
                </Button>
              </Card>
            ))
          )}
      ...
      <main className="content">
        <div className="page-heading">
          <div>
            ...
            <h1>Ein Plan, der zu dir passt.</h1>
            <p>Starte übersichtlich und passe später jede Übung an, {displayName}.</p>
          </div>
        </div>
        {error && (
        ...
        <Card className="stack empty-feature-card">
          ...
          <div>
            <h2>Eigenen Plan erstellen</h2>
            <p className="muted">
              Wähle deinen Split, benenne Trainingstage und lege Übungen, Reihenfolge und Sätze selbst fest.
            </p>
          </div>
          <Button full onClick={() => setBuilder({ template: false })}>
        ...
        <Card className="card--soft card--interactive" onClick={() => setView('templates')}>
          <div className="card__row">
            <div className="row">
              <Library size={21} />
              <div>
                <h3>Vorgefertigte Pläne</h3>
                <span className="muted small">Vorlagen ansehen oder hinzufügen</span>
              </div>
            </div>
  ...
  return (
    <main className="content training-dashboard">
      <div className="page-heading">
        <div>
          <span className="eyebrow">Aktiver Trainingsplan</span>
          <h1>{activePlan.name}</h1>
        </div>
        <IconButton label="Trainingsplan bearbeiten" onClick={() => setBuilder({ plan: activePlan, template: false })}>
          <Pencil size={19} />
        </IconButton>
      </div>
      {error && (
        ...
        <Card className="day-workspace stack">
          <div className="day-workspace__top">
            <div>
              ...
              <p>
                {selectedDayExercises.length} Übungen ·{' '}
                {selectedDayExercises.reduce((sum, exercise) => sum + exercise.target_sets, 0)} Sätze
              </p>
            </div>
          ...
          <div className="plan-exercise-list">
            {selectedDayExercises.map((exercise, index) => (
              <div className="plan-exercise-row" key={exercise.id}>
                ...
                <small>
                  {exercise.target_sets} {exercise.target_sets === 1 ? 'Satz' : 'Sätze'}
                </small>
              </div>
            ))}
          </div>
      ...
      <div className="row row--between training-footer-actions">
        <Button variant="ghost" onClick={() => setView('templates')}>
          <Library size={17} /> Planvorlagen
        </Button>
        <Button variant="ghost" onClick={() => setBuilder({ plan: activePlan, template: false })}>
          <Pencil size={17} /> Plan bearbeiten
        </Button>
      </div>
      {activePlan.notes && (
        <Card className="card--soft">
          <span className="eyebrow">Notiz zum Plan</span>

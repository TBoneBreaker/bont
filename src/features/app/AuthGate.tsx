...
export function AuthGate({ demoMode, children }: { demoMode: boolean; children: (context: AuthContext) => ReactNode }) {
  ...
  const [auth, setAuth] = useState<{ ready: boolean; session: Session | null; error: string; key: string }>({
    ready: initialReady,
    session: null,
    error: '',
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(({ data, error }) => {
      if (!active) return
      setAuth({ ready: true, session: data.session, error: error ? getUserMessage(error, 'Die Anmeldung konnte nicht geladen werden.') : '', key })
    }).catch((error: unknown) => {
      if (active) setAuth({ ready: true, session: null, error: getUserMessage(error, 'Die Anmeldung konnte nicht geladen werden.'), key })
    })
    const { data } = supabase.auth.onAuthStateChange((_event, session) => {
      if (active) setAuth({ ready: true, session, error: '', key })
    })
    return () => {
      active = false

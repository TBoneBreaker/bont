...
export function MealList({ meals, entries, onAdd, onRemove }: {
  meals: MealSlot[]
  entries: FoodEntry[]
        ...
        return (
          <Card key={meal.id} className="stack meal-card">
            <div className="card__row">
              ...
              <IconButton label={`${meal.name}: Lebensmittel hinzufügen`} onClick={() => onAdd(meal)}><Plus size={20} /></IconButton>
            </div>
            {foods.length > 0 && <div className="food-list">
              {foods.map((food) => (
                <div className="food-row" key={food.id}>
                  ...
                  <IconButton label={`${food.name} entfernen`} onClick={() => onRemove(food)}><Trash2 size={16} /></IconButton>
                </div>
              ))}
            </div>}
            <button className="add-row" onClick={() => onAdd(meal)}><Plus size={17} /> Lebensmittel hinzufügen</button>
          </Card>
        )
      })}
    </div>

        ...
        {meals.map((meal) => (
          <div className="meal-edit-row" key={meal.id}>
            <input
              ...
              maxLength={40}
              onBlur={(event) => {
                void rename(meal, event.currentTarget.value)
                event.currentTarget.value = event.currentTarget.value.trim()
              }}
            />
            <IconButton label="Mahlzeit entfernen" disabled={meals.length <= 1} onClick={() => void remove(meal)}>
              <Trash2 size={17} />
            </IconButton>
          </div>
        ))}
      </div>
      <Button variant="secondary" full disabled={meals.length >= 10} onClick={() => void add()}>
        <Plus size={18} /> Mahlzeit hinzufügen ({meals.length}/10)
      </Button>

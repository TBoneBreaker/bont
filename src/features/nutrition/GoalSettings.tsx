...
export function GoalSettings({
  ...
  return (
    <Card className="stack">
      <div>
        <span className="eyebrow">Zielrichtung</span>
        <h2>Was ist dein aktuelles Ziel?</h2>
      </div>
      <div className="segmented">
        <button aria-pressed={settings?.goal_mode === 'cut'} onClick={() => onGoalChange('cut')}>
          Defizit
        </button>
        <button aria-pressed={settings?.goal_mode === 'maintain'} onClick={() => onGoalChange('maintain')}>
          Halten
        </button>
        <button aria-pressed={settings?.goal_mode === 'bulk'} onClick={() => onGoalChange('bulk')}>
          Aufbau
        </button>
      </div>

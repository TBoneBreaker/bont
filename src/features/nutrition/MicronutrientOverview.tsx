...
export function MicronutrientOverview({
  ...
  return (
    <Card className="stack micronutrient-card">
      <div className="section-heading section-heading--inside"><div><span className="eyebrow">Tagesübersicht</span><h2>Mikronährstoffe</h2></div><span className="pill">{availableCount}/{nutrientReferences.length} mit Daten</span></div>
        ...
        {nutrientReferences.map((nutrient) => {
          ...
          return (
            <div className={`nutrient-row ${isAvailable ? '' : 'nutrient-row--unknown'}`} key={nutrient.key}>
              <button className="nutrient-row__info" onClick={() => onInfo(nutrient)} aria-label={`Information zu ${nutrient.label}`}><Info size={16} /></button>

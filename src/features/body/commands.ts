...
function validateMetric(metric: BodyMetric, value: number | null) {
  ...
  const valid = metric === 'weight_kg' ? value >= 35 && value <= 300 : metric === 'calories' ? value >= 0 && value <= 10_000 : value >= 0 && value <= 100_000
  if (!Number.isFinite(value) || !valid) throw new UserFacingError('Der Körperwert liegt außerhalb des zulässigen Bereichs.')
}

export async function saveBodyMetric({ userId, entryDate, metric, value }: { userId: string; entryDate: string; metric: BodyMetric; value: number | null }) {

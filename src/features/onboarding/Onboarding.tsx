    ...
    <main className="onboarding">
      ...
      <section className="onboarding__body">
        ...
        {step === 1 && (
          <>
            <div>
              <span className="eyebrow">Deine Basis</span>
              <h1>Ein paar Werte für den Start.</h1>
            </div>
            <p>
              Damit schätzen wir dein erstes Kalorienziel. Mit deinen echten Verlaufsdaten wird Bont später genauer.
            </p>
            <Field
              label="Geburtsdatum"
              type="date"
            ...
            />
            <SelectField
              label="Biologisches Geschlecht für die Berechnung"
              value={sex}
              onChange={(event) => {
        ...
        {step === 2 && (
          <>
            <div>
              <span className="eyebrow">Dein Alltag</span>
              <h1>Wie aktiv bist du wirklich?</h1>
            </div>
            ...
            <div className="choice-list">
              {activityOptions.map((option) => (
                <button
                  ...
                  onClick={() => setActivity(option.id as ActivityLevel)}
                >
                  <div>
        ...
        {step === 3 && (
          <>
            <div>
              <span className="eyebrow">Körperzusammensetzung</span>
              <h1>Welche Kategorie passt am ehesten?</h1>
            </div>
            <div className="choice-list">
              {bodyFatOptions[sex].map(([id, title, text]) => (
                <button
                  className="choice"
                  key={id}
                  aria-pressed={bodyFat === id}
                  onClick={() => setBodyFat(id as BodyFatCategory)}
                >
                  <div>
                    <strong>{title}</strong>
                    <span>{text}</span>
            ...
            <InfoNote>
              Die Werte liefern nur eine vorläufige Schätzung und keine medizinische Beurteilung. Du kannst alle Angaben
      ...
      <div className="row">
        {step > 0 && (
          <Button variant="secondary" onClick={() => setStep((value) => value - 1)} aria-label="Zurück">
            <ArrowLeft size={18} />
          </Button>
        )}
        <Button
          ...
          onClick={() => (step < 3 ? setStep((value) => value + 1) : void finish())}
        >
          {saving ? (
            'Wird gespeichert …'
          ) : step < 3 ? (

...
export function AuthScreen({ initialError = '' }: { initialError?: string }) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (mode === 'reset') {
      setLoading('reset')
      const { error: authError } = await authService.sendPasswordReset(
        email.trim(),
  ...
  return (
    <main className="auth-layout">
      <section className="auth-hero">
        ...
        <p>
          Bont speichert Änderungen sofort lokal und gleicht sie sicher mit deinem Konto ab, sobald Internet verfügbar
          ist.
        </p>
      </section>
      ...
      {sent ? (
        <section className="auth-panel card">
          ...
          <div>
            <h2>
              {sent === 'registration'
                ? 'E-Mail bestätigen'
                : sent === 'password-reset'
                  ? 'E-Mail zum Zurücksetzen ist unterwegs'
                  : 'Login-Link ist unterwegs'}
            </h2>
            <p className="muted">
              {sent === 'registration' ? (
                <>
                  Falls <strong>{email}</strong> neu ist, wurde eine Bestätigung angefordert. Prüfe auch Spam. Kommt
                  nichts an, nutze „Anmelden“ – die Adresse kann bereits zu einem Konto gehören.
                </>
              ) : sent === 'password-reset' ? (
                <>
                  Wenn ein Konto zu <strong>{email}</strong> gehört, erhältst du einen Link zum Festlegen eines neuen
                  Passworts.
                </>
              ) : (
                <>
          ...
          <Button variant="secondary" full onClick={() => setSent(null)}>
            Zurück
          </Button>
          <Button type="button" variant="ghost" full onClick={() => window.location.assign('/demo')}>
            Ohne Konto testen
          </Button>
        </section>
      ) : (
        <form className="auth-panel" onSubmit={submit}>
            ...
            <p>
              {mode === 'reset'
                ...
                : mode === 'login'
                  ...
                  : 'Deine Einträge werden deinem persönlichen Konto zugeordnet.'}
            </p>
          </div>
          ...
          {mode === 'register' && (
            <p className="auth-note">
              Schon einmal mit dieser E-Mail registriert? Dann „Anmelden“ wählen. Für bestehende Konten wird keine neue
              Registrierungs-Mail verschickt.
            </p>
          )}
          ...
          <Button
            ...
            disabled={
              Boolean(loading) ||
              !email.trim() ||
              (mode !== 'reset' && (mode === 'register' || loginMethod === 'password') && password.length < 8)
            }
          >
            {mode === 'reset' ? (
              <LockKeyhole size={18} />
          ...
          {mode === 'login' && (
            <Button
              ...
              onClick={() => {
                setLoginMethod((method) => (method === 'magic-link' ? 'password' : 'magic-link'))
                setPassword('')
                setError('')
              }}
            >
              ...
              {loginMethod === 'magic-link'
                ...
                : 'Ohne Passwort per E-Mail-Link anmelden'}
            </Button>
          )}
          {mode === 'login' && loginMethod === 'password' && (
            <Button type="button" variant="ghost" full disabled={Boolean(loading)} onClick={() => changeMode('reset')}>
              Passwort vergessen?
            </Button>
          )}
          {mode === 'reset' && (
          ...
          <p className="auth-note">
            Altes Bont-Konto ohne Passwort? Nutze den Anmeldelink. Deine bisherigen Daten bleiben erhalten.
          </p>
        </form>
      )}
    </main>

...
describe('sync conflict policy', () => {
  it('uses timestamp and id as a deterministic last-write-wins order', () => {
    expect(
      remoteWins(
        { ...base, updated_at: '2026-01-01T00:00:00.000Z' },
        { ...base, id: 'b', updated_at: '2026-01-01T00:00:00.000Z' },
      ),
    ).toBe(true)
    expect(
      remoteWins(
        { ...base, updated_at: '2026-01-02T00:00:00.000Z' },
        { ...base, updated_at: '2026-01-01T00:00:00.000Z' },
      ),
    ).toBe(false)
  })
  ...
  it('merges independent body metrics from a pending local row', () => {
    ...
    expect(mergeBodyEntries(undefined, remote, pending)).toMatchObject({
      id: 'remote',
      weight_kg: 79,
      calories: 2500,
      steps: 9000,
  ...
  it('serializes the composite cursor and advances equal timestamps by id', () => {
    ...
    expect(cursorFromRows([{ updated_at: cursor.updated_at, id: 'b' }], cursor)).toEqual({
